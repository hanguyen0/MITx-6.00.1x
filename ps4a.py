...
#HAND_SIZE = 7
#make call to dealHand(n) and playHand
    #this function can be written in about 15-20 lines of code
    #if user input is not n or r or e, then print invalid command and repeat user input prompt
    count=0
    while True:
        userInput=input('Enter n to deal a new hand, r to replay the last hand, or e to end game: ') 
        #if user input is n, start a new Hand
        if userInput=='n':
            ...
            previous=hand.copy()
        #if user input is r:
            #1)if entered r before n, print You have not played a hand yet
            #2)if entered r after n, then repeat that Hand
        elif userInput=='r':
            if count==0:
                print('You have not played a hand yet. Please play a new hand first!')
            else:
                playHand(previous, wordList, HAND_SIZE)
        #if user input is e: end game, exit function, no return
        elif userInput=='e':
            break
        else:
